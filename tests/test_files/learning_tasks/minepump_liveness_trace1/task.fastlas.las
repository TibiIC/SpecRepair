...
#modeb(2,timepoint_of_op(const(temp_op_v), var(time), var(time), var(trace))).
#modeb(2,holds_at(const(usable_atom), var(time), var(trace))).
#modeb(2,not_holds_at(const(usable_atom), var(time), var(trace))).
usable_atom(flag).
usable_atom(highwater).
usable_atom(methane).
usable_atom(pump).
index(0..1).
temp_op_v(current).
temp_op_v(next).
temp_op_v(prev).
temp_op_v(eventually).
expression_v(assumption3_1).
#bias("
:- in_head(antecedent_exception(_,_,V1,V2)), in_body(timepoint_of_op(_,V3,_,V4)), (V1, V2) != (V3, V4).
:- in_head(antecedent_exception(_,_,_,V1)), in_body(holds_at(_,_,V2)), V1 != V2.
:- in_head(antecedent_exception(_,_,_,V1)), in_body(not_holds_at(_,_,V2)), V1 != V2.
:- in_body(holds_at(E1, _, _)), in_body(holds_at(E2, _, _)), E1 != E2.
:- in_body(holds_at(_, _, _)), in_body(not_holds_at(_, _, _)).
:- in_body(not_holds_at(_, _, _)), in_body(holds_at(_, _, _)).
:- in_body(not_holds_at(E1, _, _)), in_body(not_holds_at(E2, _, _)), E1 != E2.
...
:- in_head(antecedent_exception(_,_,_,_)), in_body(timepoint_of_op(next,_,_,_)).
:- in_head(antecedent_exception(_,_,_,_)), in_body(timepoint_of_op(eventually,_,_,_)).
:- in_head(antecedent_exception(_,_,_,_)), in_body(timepoint_of_op(prev,_,_,_)).
:- in_head(antecedent_exception(_,_,_,_)), in_body(timepoint_of_op(next,_,_,_)), in_body(holds_at(flag,_,_)).
:- in_head(antecedent_exception(_,_,_,_)), in_body(timepoint_of_op(next,_,_,_)), in_body(not_holds_at(flag,_,_)).
:- in_head(antecedent_exception(_,_,_,_)), in_body(timepoint_of_op(next,_,_,_)), in_body(holds_at(pump,_,_)).
:- in_head(antecedent_exception(_,_,_,_)), in_body(timepoint_of_op(next,_,_,_)), in_body(not_holds_at(pump,_,_)).
").
