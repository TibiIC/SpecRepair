...
# Variable partition (comma-separated, no spaces)
INPUTS="methane,highwater"
OUTPUTS="pump,state"

# Environment assumptions
INIT_ENV="!highwater&!methane"
INVARIANT_ENV="(pump&X(pump) -> XX(!highwater))&(pump->(!highwater|!methane))"
JUSTICE_ENV=()

# System guarantees
INIT_SYS="!pump&!state"
INVARIANT_SYS="(highwater -> X(pump))& (methane -> (X(!pump) | X(methane)))&(methane -> (X(!pump) | X(highwater)))&(!state & (!(methane) | ((methane) & (X(!pump)))) & X(!state)) | (!state & ((methane) & !(X(!pump))) & X(state)) | (state & (X(!pump)) & X(!state)) | (state & !(X(!pump)) & X(state))"
JUSTICE_SYS=("!state")
...
COND_2="(($INIT_ENV) -> (($INVARIANT_SYS) W !($INVARIANT_ENV)))"
