...
root_antecedent_holds(OP,assumption1_1,0,T,S):-
	trace(S),
	timepoint(T,S),
	temporal_operator(OP),
	holds_at(OP,pump,T,S).

root_antecedent_holds(OP,assumption1_1,1,T,S):-
	trace(S),
	timepoint(T,S),
	temporal_operator(OP),
	holds_at(OP,pump,T,S).
...
antecedent_holds(assumption2_1,T,S):-
	trace(S),
	timepoint(T,S),
	root_antecedent_holds(OP,assumption2_1,0,T,S).
...
root_consequent_holds(OP,assumption2_1,1,T,S):-
	trace(S),
	timepoint(T,S),
	temporal_operator(OP),
