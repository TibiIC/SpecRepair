% Generated from consequent_exception.las by scripts/ilasp_to_fastlas.py -- edit that file.
% Run:  FastLAS --space-size consequent_exception_fastlas.las      (check the space first)
%       FastLAS consequent_exception_fastlas.las
%
% - dropped the #ilasp_script solution-enumeration driver (FastLAS has no pylasp; see the header of the output)
% - rewrote 5 head()/body() literal(s) in #bias to in_head()/in_body() -- untranslated these are silently inert
% - gave 1 example(s) an identifier, which FastLAS requires
% - moved 7 #constant declaration(s) into t/1 facts
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Consequent exception
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Widen the consequent so the rule has another way to be discharged.
%
...
% The ILASP pylasp driver was dropped here: FastLAS has no
% equivalent and returns ONE hypothesis per invocation.
% See scripts/fastlas_solutions.py for how to get the other nine.

% The consequent is a disjunction of conjunctions. Adding a whole new DISJUNCT
% gives the implication a second way to hold, so the formula is again WEAKER -
% but for a different reason, and it admits a different set of traces.
%
%     G( a -> b )   becomes   G( a -> b | <new> )
%
% Note the asymmetry with the antecedent case: there a conjunct was added to an
% existing disjunct, here a disjunct is added to the parent. Both weaken, and in
% the AST both are one attachment. In the GR(1) encoding they needed two
% unrelated predicates and two sets of rules.
%
% Run:  ILASP --version=4 files/until_semantics/consequent_exception.las
...
#modeh(disjunction(const(cons_node), const(spare))).
#modeh(conjunction(const(spare), const(spare))).
...
% #constant(atom,a).   -> `atom(a).` in the background
% #constant(atom,b).   -> `atom(b).` in the background
% #constant(atom,c).   -> `atom(c).` in the background
...
% #constant(spare,x0).   -> `spare(x0).` in the background
% #constant(spare,x1).   -> `spare(x1).` in the background
% A third spare, because the consequent needs one more node than the antecedent
% for the same repair: the new disjunct must be a conjunction before it can hold
% a literal, so `b | prev(a)` is disjunction -> conjunction -> previous -> atomic,
% three spares, where the antecedent's `a & prev(b)` is conjunction -> previous
% -> atomic and needs two. With only two declared, every temporal repair here was
% unreachable and the task would have quietly learned only bare literals.
% #constant(spare,x2).   -> `spare(x2).` in the background
...
% #constant(cons_node,f_consequent).   -> `cons_node(f_consequent).` in the background
#bias("
...
:- in_head(disjunction(N,N)).
:- in_head(conjunction(N,N)).
:- in_head(negate(N,N)).
:- in_head(previous(N,N)).
:- in_head(w_next(N,N)).
% The attachment point is already pinned by const(cons_node) above, which
...
kind_of(N,conjunction) :- spare(N), conjunction(N,_).
...
% repair degenerates into deleting whatever it was attached to. Checking every
% pointing predicate catches a dangle at any depth, not just at the attachment.

referenced(N) :- disjunction(_,N), spare(N).
...
points(N,M) :- conjunction(N,M), spare(N), spare(M).

reaches(N,M) :- points(N,M).
reaches(N,M) :- reaches(N,K), points(K,M).
:- spare(N), reaches(N,N).

attached(N) :- cons_node(C), disjunction(C,N), spare(N).
...
#pos(consequent_exception_eg1, {sat(g1)}, {}, {
  trace_name(g1).
  time(0..1,g1).
  trace(1,a,g1).
}).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Constant pools (was #constant)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% FastLAS grounds every const(t)/var(t) placeholder from a t/1
% predicate here. Without one the space is empty and FastLAS
% reports UNSATISFIABLE with no error -- indistinguishable from
% a task that genuinely has no solution. Duplicates are harmless.
