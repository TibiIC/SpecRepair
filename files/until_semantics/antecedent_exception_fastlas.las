% Generated from antecedent_exception.las by scripts/ilasp_to_fastlas.py -- edit that file.
% Run:  FastLAS --space-size antecedent_exception_fastlas.las      (check the space first)
%       FastLAS antecedent_exception_fastlas.las
%
% - dropped the #ilasp_script solution-enumeration driver (FastLAS has no pylasp; see the header of the output)
% - rewrote 4 head()/body() literal(s) in #bias to in_head()/in_body() -- untranslated these are silently inert
% - gave 1 example(s) an identifier, which FastLAS requires
% - moved 6 #constant declaration(s) into t/1 facts
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% #constant(ante_conjunct,f_antecedent_conjunct_0).   -> `ante_conjunct(f_antecedent_conjunct_0).` in the background
#bias("
...
#pos(antecedent_exception_eg1, {sat(g1)}, {}, {
  trace_name(g1).
  time(0..1,g1).
  trace(1,a,g1).
}).
