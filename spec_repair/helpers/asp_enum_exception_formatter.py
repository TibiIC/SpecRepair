...
class ASPEnumExceptionFormatter(ILTLFormatter):
    def __init__(
            self,
    ...
    def format_exp(self, this_formula: LTLFormula) -> str:
        ...
        match this_formula:
            case AtomicProposition(name=name, value=value):
                if value is True:
                    return f"holds_at({name},true,T2,S)"
                elif value is False:
                    return f"holds_at({name},false,T2,S)"
                else:
                    return f"holds_at({name},{str(value).lower()},T2,S)"
