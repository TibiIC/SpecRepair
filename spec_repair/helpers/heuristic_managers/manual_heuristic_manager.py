...
from spec_repair.heuristics import first_choice, choose_one_with_heuristic, manual_choice


class ManualHeuristicManager(IHeuristicManager):
    ...
    def select_alternative_learning_tasks(self, ctss: List[List[CounterTrace]]) -> List[List[CounterTrace]]:
        return [choose_one_with_heuristic(ctss, manual_choice)]

    def select_possible_learning_adaptations(self, adaptations: List[List[str]]) -> List[List[str]]:
        return [choose_one_with_heuristic(adaptations, manual_choice)]
